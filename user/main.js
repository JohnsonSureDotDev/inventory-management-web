let sellIsValid=0;
const parentHtml=document.getElementsByClassName("appendToMe")[0];//use id
...
const cspActive=document.getElementById("cspActive")//custom selling price(csp)
const formDatabase= [];
//fetching from db
    ...
    function createElmAndContent(a,b,c,d,e,/* f,g,h,i */j){
            const tr=document.createElement("tr")
             tr.setAttribute("class","row");
              if(parentHtml.childElementCount===10-1){
                //max items to display to prevent browser crash
                let info=`......${  formDatabase.length-10} other items`
                a=info
                b=info
                c=info
                d=info
              ...
              if(parentHtml.childElementCount===10){
                     return;
              }
...
VALUES (
 ...
 sell.disabled=true
  ...
  if((isNaN(Number(elementCreated.parentElement.children[0].textContent+1)))){ 
    elementCreated.parentElement.removeChild(elementCreated)
   } 
  ...
  //add menu icon 1
  //add hover effect on individual items 2 
  add loading skeleton for smooth transitions

  
  add login interface
  buld admin intrerface with data visualizers
  reduce lots of browswer refreshing
  */
